...
int EffectManager::PlayEffect(Vector3 position, Quaternion rotation, Vector3 scale, int num)
{
    ...
    effect->Play();
    m_effect.emplace(ID,effect);

    return ID++;
}

void EffectManager::StopEffect(int id)
{
    m_effect[id]->Stop();
    DeleteGO(m_effect[id]);
    m_effect.erase(m_effect.find(id));
}

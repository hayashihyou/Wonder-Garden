...
public:
    UIBase() { m_uiAnimationList.clear(); }
    virtual ~UIBase()
    {
        // 明示的に消しているだけ。本来は要らない
...
public:
    void UpdateAnimation()
    {
        ...
        ForEachAnimation([](UIAnimationBase* animation) { animation->Update(); });
    }
    void PlayAnimation()
    {
        ...
        ForEachAnimation([](UIAnimationBase* animation) { animation->Play(); });
    }
    ...
    {
        auto it = std::find_if(m_uiAnimationList.begin(), m_uiAnimationList.end(),
    ...
    {
        ForEachAnimation([](UIAnimationBase* animation) { animation->Stop(); });
    }
    ...
    void RemoveAnimation(const uint32_t key) { m_uiAnimationList.erase(key); }
...
public:
    void Initialize(const char* assetName, const float width, const float height, const Vector3& position,
                    const Vector3& scale, const Quaternion& rotation);
};
...
public:
    ...
    void SetText(const wchar_t* text) { m_fontRender.SetText(text); }
};
...
public:
     ...
     * ・回転
     */
    void Initialize(const char* assetPath, const int digit, const int number, const float widht, const float height,
...
public:
    template <typename T> T* CreateUI(const uint32_t key)
    {
    ...
    void RemoveUI(const uint32_t key) { m_uiList.erase(key); }

    template <typename T> T* FindUI(const uint32_t key)
    {
